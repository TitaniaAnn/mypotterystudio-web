...
class GitHubAPI {
    ...
    public static function getIssue(string $repo, string $token, int $issueNumber): array {
        if (!$repo || !$token || !$issueNumber) return [];
        $url = "https://api.github.com/repos/{$repo}/issues/{$issueNumber}";
        return self::get($url, $token);
    }

    public static function updateIssue(string $repo, string $token, int $issueNumber, array $data): array {
        if (!$repo || !$token || !$issueNumber) return [];
        $url = "https://api.github.com/repos/{$repo}/issues/{$issueNumber}";
        $ch = curl_init($url);
        curl_setopt_array($ch, [
            ...
            CURLOPT_TIMEOUT => 10,
            CURLOPT_HTTPHEADER => [
                'User-Agent: MyPotteryStudio/1.0',
